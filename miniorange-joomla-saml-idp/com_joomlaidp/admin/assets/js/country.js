/**
 * Country list with dial codes for phone input dropdown
 */
var moIdpCountries = [
...
/**
 * Populate country code dropdown. Call after DOM ready.
 * @param {string} selectId - ID of the select element (e.g. 'mo_saml_country_code')
 * @param {string} defaultDial - Optional default dial code to select (e.g. '91' for India)
 */
function moIdpPopulateCountryCodeSelect(selectId, defaultDial) {
    var sel = document.getElementById(selectId);
    if (!sel || typeof moIdpCountries === 'undefined') return;
    var i, opt, dialVal;
    for (i = 0; i < moIdpCountries.length; i++) {
        opt = document.createElement('option');
        dialVal = '+' + moIdpCountries[i].dial;
        opt.value = dialVal;
        opt.textContent = moIdpCountries[i].name + ' (' + dialVal + ')';
        sel.appendChild(opt);
        if (defaultDial && moIdpCountries[i].dial === String(defaultDial)) {
            opt.selected = true;
        }
    }
}
